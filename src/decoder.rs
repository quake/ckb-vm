...
impl Decoder {
    ...
    fn decode_mop_inner<M: Memory>(&mut self, memory: &mut M, pc: u64) -> Result<Instruction, Error> {
    ...
    fn decode_raw_without_cache<M: Memory>(&mut self, memory: &mut M, pc: u64) -> Result<Instruction, Error> {
        let instruction_bits = self.decode_bits(memory, pc)?;
        for factory in &self.factories {
            if let Some(instruction) = factory(instruction_bits, self.version) {
                return Ok(instruction);
